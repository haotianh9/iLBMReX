...
void AmrCoreLBM::Evolve() {
  ...
  for (int step = istep[0]; step < max_step && cur_time < stop_time; ++step) {
    ...
    //         amrex::Print() << "Coarse STEP " << step+1 << " ends." << " TIME
    //         = " << cur_time
...
void AmrCoreLBM::MakeNewLevelFromCoarse(int lev, Real time, const BoxArray &ba,
  ...
  const int ncomp = f_new[lev - 1].nComp();
  const int nghost = f_new[lev - 1].nGrow();
  amrex::Print() << "Making new level " << lev
                 << " from coarse, ncomp = " << ncomp << std::endl;
  f_new[lev].define(ba, dm, ncomp, nghost);
  f_old[lev].define(ba, dm, ncomp, nghost);
  ...
  // if (lev > 0 && do_reflux) {
  //     flux_reg[lev].reset(new FluxRegister(ba, dm, refRatio(lev-1), lev,
  //     ncomp));
  // }
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
  ...
  for (MFIter mfi(rho_fab, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
    ...
    amrex::launch(box, [=] AMREX_GPU_DEVICE(Box const &tbx) {
      initdata(tbx, fab_rho, fab_ux, fab_uy, problo, probhi, dx, nu);
    });
  }
}
...
void AmrCoreLBM::ErrorEst(int lev, TagBoxArray &tags, Real /*time*/,
  ...
  // amrex::Print() << "Tagging cells for refinement, level: " << lev <<
  // std::endl; only do this during the first call to ErrorEst
  if (first) {
    first = false;
  ...
  // for now as a place holder, I used density fluctuation as the indicator for
  // refinement,
#ifdef AMREX_USE_OMP
#pragma omp parallel if (Gpu::notInLaunchRegion())
#endif
...
void AmrCoreLBM::AverageDown() {
  for (int lev = finest_level - 1; lev >= 0; --lev) {
    ...
    amrex::average_down(ux[lev + 1], ux[lev], geom[lev + 1], geom[lev], 0,
                        ux[lev].nComp(), refRatio(lev));
    amrex::average_down(uy[lev + 1], uy[lev], geom[lev + 1], geom[lev], 0,
                        uy[lev].nComp(), refRatio(lev));
  }
}
...
void AmrCoreLBM::AverageDownTo(int crse_lev) {
  ...
  amrex::average_down(rho[crse_lev + 1], rho[crse_lev], geom[crse_lev + 1],
                      ...
                      refRatio(crse_lev));
  amrex::average_down(ux[crse_lev + 1], ux[crse_lev], geom[crse_lev + 1],
                      geom[crse_lev], 0, ux[crse_lev].nComp(),
                      refRatio(crse_lev));
  amrex::average_down(uy[crse_lev + 1], uy[crse_lev], geom[crse_lev + 1],
                      geom[crse_lev], 0, uy[crse_lev].nComp(),
                      refRatio(crse_lev));
}
...
void AmrCoreLBM::timeStepWithSubcycling(int lev, Real time, int iteration) {
  ...
  if (lev < finest_level) {
    ...
    if (do_reflux) {
      ...
      flux_reg[lev + 1]->Reflux(f_new[lev], 1.0, 0, 0, f_new[lev].nComp(),
                                geom[lev]);
    }
...
void AmrCoreLBM::timeStepNoSubcycling(Real time, int iteration) {
...
void AmrCoreLBM::WriteCheckpointFile() const {
  ...
  //     (e.g., finest_level, t_new, etc.) and also
  ...
  //     amrex::Concatenate(chk_file,istep[0]);
  ...
  //     amrex::PreBuildDirectorHierarchy(checkpointname, "Level_", nlevels,
  //     true);
  ...
  //        io_buffer.size()); HeaderFile.open(HeaderFileName.c_str(),
  //        std::ofstream::out   |
  //                                                std::ofstream::trunc |
  //                                                std::ofstream::binary);
  //        if( ! HeaderFile.good()) {
...
void AmrCoreLBM::ReadCheckpointFile() {
  ...
  //     {
  //         facevel[lev][idim] =
  //         MultiFab(amrex::convert(ba,IntVect::TheDimensionVector(idim)), dm,
  //         1, 1);
  //     }
  // }
...
void AmrCoreLBM::InitEquilibrium() {
  // amrex::Print() << "InitEquilibrium: " <<finest_level <<"\n";
  for (int lev = finest_level; lev >= 0; --lev) {
    // amrex::Print() << "InitEquilibrium: " << lev << "\n";
    MultiFab &ux_fab = ux[lev];
    MultiFab &uy_fab = uy[lev];
    MultiFab &rho_fab = rho[lev];
    ...
    for (MFIter mfi(f_old_fab, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
      ...
      amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j,
        ...
        for (unsigned int i_dir = 0; i_dir < ndir; ++i_dir) {
          double cidotu =
              dirx[i_dir] * ux_array(i, j, k) + diry[i_dir] * uy_array(i, j, k);
