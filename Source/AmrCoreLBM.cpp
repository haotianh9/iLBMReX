...
AmrCoreLBM::AmrCoreLBM ()
{
    ...
    nsubsteps.resize(nlevs_max, 1); 
        ...
        for (int lev = 1; lev <= max_level; ++lev) {nsubsteps[lev] = MaxRefRatio(lev-1);}
    ...
    for (int lev = 0; lev <= max_level; ++lev) {xCellSize[lev]=geom[lev].CellSize(0); yCellSize[lev]=geom[lev].CellSize(1); zCellSize[lev]=geom[lev].CellSize(2);}
	...
	for (int lev = 1; lev <= max_level; ++lev) {dt[lev] = xCellSize[lev];	tau[lev]=0.5 + nsubsteps[lev]*(tau[lev-1] - 0.5); }
...
void
AmrCoreLBM::MakeNewLevelFromCoarse (int lev, Real time, const BoxArray& ba,
                                    const DistributionMapping& dm)
{
  ...
  //This part requires special attention, both macroscopic - mesoscopic need to be initialized	
...
// overrides the pure virtual function in AmrCore
void
AmrCoreLBM::RemakeLevel (int lev, Real time, const BoxArray& ba,
                         const DistributionMapping& dm)
{
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
  ...
  for (MFIter mfi(curMacro,TilingIfNotGPU()); mfi.isValid(); ++mfi) {
    ...
    amrex::ParallelFor(gtbx, [=] AMREX_GPU_DEVICE(int i,int j,int k) noexcept {
	if(vbx.contains(i,j,k)){ visPara(i,j,k,rho,u,v,vor,P,tempdx,tempdy,T0);}
	});
}
...
AmrCoreLBM::ReadParameters ()
{
    ...
    
    {
        ...
        wi.resize(wis.size());
        for(size_t idvs=0; idvs<wis.size(); idvs++){wi[idvs] = wis[idvs]/wim[idvs];}
		...
		amrex::Gpu::copy(amrex::Gpu::hostToDevice,wi.begin(),wi.end(),wi_dev.begin());
		amrex::Gpu::copy(amrex::Gpu::hostToDevice,dirx.begin(),dirx.end(),dirx_dev.begin());
		amrex::Gpu::copy(amrex::Gpu::hostToDevice,diry.begin(),diry.end(),diry_dev.begin());
		amrex::Gpu::copy(amrex::Gpu::hostToDevice,dirz.begin(),dirz.end(),dirz_dev.begin());
		
		const Geometry& geom = Geom(0);
		L0 = geom.ProbHi(0) - geom.ProbLo(0);
		
		//The characteristic velocity is hard-coded when it is initialized, so I will define it here directly.
		U0=0.02;		
		T0 = L0/U0;
...
void
AmrCoreLBM::AverageDown()
{
    for (int lev = finest_level-1; lev >= 0; --lev)
    {
        amrex::average_down(f_new[lev+1], f_new[lev],geom[lev+1], geom[lev],0, f_new[lev].nComp(), refRatio(lev));
        amrex::average_down(macro_new[lev+1], macro_new[lev],geom[lev+1], geom[lev],0, macro_new[lev].nComp(), refRatio(lev));
    }
...
void
AmrCoreLBM::AverageDownTo (int crse_lev)
{
...
AmrCoreLBM::FillPatchMesoscopic (int lev, Real time, MultiFab& mf, int icomp, int ncomp)
{
    if (lev == 0)
    {
        ...
        if(Gpu::inLaunchRegion())
        {
            ...
            PhysBCFunct<GpuBndryFuncFab<AmrCoreFill> > physbc(geom[lev],bcsMesoscopic,gpu_bndry_func);
    ...
    else
    {
        ...
        {
            GpuBndryFuncFab<AmrCoreFill> gpu_bndry_func(AmrCoreFill{});
            PhysBCFunct<GpuBndryFuncFab<AmrCoreFill> > cphysbc(geom[lev-1],bcsMesoscopic,gpu_bndry_func);
            PhysBCFunct<GpuBndryFuncFab<AmrCoreFill> > fphysbc(geom[lev],bcsMesoscopic,gpu_bndry_func);
        ...
        {
            CpuBndryFuncFab bndry_func(nullptr);  // Without EXT_DIR, we can pass a nullptr.
            PhysBCFunct<CpuBndryFuncFab> cphysbc(geom[lev-1],bcsMesoscopic,bndry_func);
...
void
AmrCoreLBM::timeStepWithSubcycling (int lev, Real time, int iteration)
{
...
// get plotfile name
std::string
AmrCoreLBM::PlotFileName (int lev) const
{
...
amrex::Vector<std::unique_ptr<amrex::MultiFab>> AmrCoreLBM::PlotFileMF() const {
  ...
  for (int lev = 0; lev <= finest_level; ++lev) {
    ...
    plot_mfs.push_back(std::make_unique<amrex::MultiFab>(grids[lev], dmap[lev], totalNumberVar, 0));

    // Copy rho
    amrex::MultiFab::Copy(*plot_mfs[lev], macro_new[lev], rho_comp, rho_comp, 1, 0);

    // Copy ux
    amrex::MultiFab::Copy(*plot_mfs[lev], macro_new[lev], ux_comp,  ux_comp, 1, 0);

    // Copy uy
    amrex::MultiFab::Copy(*plot_mfs[lev], macro_new[lev], uy_comp, uy_comp, 1, 0);

    // Copy uz
    amrex::MultiFab::Copy(*plot_mfs[lev], macro_new[lev], uz_comp, uz_comp, 1, 0);
    
    // Copy vor
    amrex::MultiFab::Copy(*plot_mfs[lev], macro_new[lev], vor_comp, vor_comp, 1, 0);
...
void
AmrCoreLBM::WritePlotFile () const
{
...
void
AmrCoreLBM::WriteCheckpointFile () const
{
/*
   ...
   for (int lev = 0; lev <= finest_level; ++lev) {
       VisMF::Write(macro_new[lev],amrex::MultiFabFileFullPrefix(lev, checkpointname, "Level_", "phi"));
   }
*/
}
...
AmrCoreLBM::FillPatchMacro (int lev, Real time, MultiFab& mf, int icomp, int ncomp)
{
    if (lev == 0)
    {
        ...
        {
            GpuBndryFuncFab<AmrCoreFill> gpu_bndry_func(AmrCoreFill{});
            PhysBCFunct<GpuBndryFuncFab<AmrCoreFill> > physbc(geom[lev],bcsMacro,gpu_bndry_func);
            amrex::FillPatchSingleLevel(mf, time, smf, stime, 0, icomp, ncomp,
                                        geom[lev], physbc, 0);
        }
        else
    ...
    else
    {
        ...
        {
            GpuBndryFuncFab<AmrCoreFill> gpu_bndry_func(AmrCoreFill{});
            PhysBCFunct<GpuBndryFuncFab<AmrCoreFill> > cphysbc(geom[lev-1],bcsMacro,gpu_bndry_func);
            PhysBCFunct<GpuBndryFuncFab<AmrCoreFill> > fphysbc(geom[lev],bcsMacro,gpu_bndry_func);
