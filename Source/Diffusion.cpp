...
Diffusion::Diffusion(Amr *Parent, NavierStokesBase *Caller, Diffusion *Coarser,
                     int num_state, FluxRegister *Viscflux_reg,
                     const Vector<int> &_is_diffusive)
    : parent(Parent), navier_stokes(Caller), grids(navier_stokes->boxArray()),
      dmap(navier_stokes->DistributionMap()), level(navier_stokes->Level()),
      coarser(Coarser), finer(nullptr), NUM_STATE(num_state),
...

{
  if (!initialized) {
    ...
    if (n_diff < NUM_STATE)
      amrex::Abort(
          "Diffusion::Diffusion(): is_diffusive array is not long enough");
...
FluxRegister *Diffusion::viscFluxReg() { return viscflux_reg; }

int Diffusion::maxOrder() { return max_order; }

int Diffusion::tensorMaxOrder() { return tensor_max_order; }
...
Real Diffusion::get_scaled_abs_tol(const MultiFab &rhs,
...
void Diffusion::diffuse_scalar(
    const Vector<MultiFab *> &S_old, const Vector<MultiFab *> &Rho_old,
    Vector<MultiFab *> &S_new, const Vector<MultiFab *> &Rho_new, int S_comp,
    int nComp, int Rho_comp, Real prev_time, Real curr_time, Real be_cn_theta,
    const MultiFab &rho_half, int rho_flag, MultiFab *const *fluxn,
    MultiFab *const *fluxnp1, int fluxComp, MultiFab *delta_rhs, int rhsComp,
    const MultiFab *alpha_in, int alpha_in_comp, const MultiFab *const *betan,
    const MultiFab *const *betanp1, int betaComp, const IntVect &cratio,
    const BCRec &bc, const Geometry &geom, bool add_old_time_divFlux,
  ...
  if (S_comp <= Xvel && Xvel <= S_comp + nComp - 1) {
    amrex::Abort(
        "Diffusion::diffuse_scalar(): velocity component(s) attempting to use "
        "scalar solver. Velocity must use tensor solver.\n");
  }
  ...
  for (int comp = 1; comp < nComp; comp++) {
    if (a_is_diffusive[comp] != diffType) {
      amrex::Abort("All the scalars must be either diffusive or non-diffusive "
                   "when calling diffuse_scalar");
    }
  }
  ...
  if (allnull && add_old_time_divFlux && be_cn_theta != 1)
    amrex::Abort("Diffusion::diffuse_scalar: Constant diffusivity case no "
                 "longer supported separately. Must set non-zero beta.");
  ...
  const DistributionMapping *dmc =
      (has_coarse_data ? &(S_new[1]->DistributionMap()) : nullptr);
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
          ...
          for (MFIter mfi(*Solnc, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
            ...
            amrex::ParallelFor(bx, [sol_arr, rho_arr, nComp] AMREX_GPU_DEVICE(
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    if (NavierStokesBase::getRedistType() != "NoRedist") {
      ...
      single_level_weighted_redistribute(rhs_tmp, Rhs, weights, 0, nComp, geom,
                                         use_wts_in_divnc);
    } else
#endif
    {
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
  ...
  for (MFIter mfi(*S_new[0], TilingIfNotGPU()); mfi.isValid(); ++mfi) {
    ...
    auto const &rho_old =
        (rho_flag == 3) ? Rho_old[0]->const_array(mfi, Rho_comp) : dummy;
    auto const &alpha = (has_alpha) ? alpha_in->const_array(mfi, alpha_in_comp)
                                    : Soln.const_array(mfi);
    auto const &deltarhs = (has_delta_rhs)
                               ? delta_rhs->const_array(mfi, rhsComp)
                               : Soln.const_array(mfi);
    amrex::ParallelFor(
        bx, nComp,
        [rhs, solution, snew, rhoHalf, rho_old, alpha, deltarhs, has_alpha,
         has_delta_rhs, rho_flag,
         dt] AMREX_GPU_DEVICE(int i, int j, int k, int n) noexcept {
          // Add body sources
          if (has_delta_rhs) {
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(*Solnc, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
      ...
      amrex::ParallelFor(
          bx, [sol_arr, snew_arr, rho_arr, nComp,
               rho_flag] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
            for (int n = 0; n < nComp; ++n) {
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
  ...
  Real rhsscale = 1.0;
  {
    ...
    computeAlpha(alpha, scalars, a, b, &rhsscale, alpha_in, alpha_in_comp,
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
  ...
  for (MFIter mfi(*S_new[0], TilingIfNotGPU()); mfi.isValid(); ++mfi) {
    ...
    const auto &rhonew = (rho_flag == 2)
                             ? Rho_new[0]->const_array(mfi, Rho_comp)
                             : S_new[0]->const_array(mfi, sigma);
    amrex::ParallelFor(bx, nComp,
                       [snew, solution, rhonew, rho_flag] AMREX_GPU_DEVICE(
...
void Diffusion::diffuse_velocity(Real dt, Real be_cn_theta,
                                 const MultiFab &rho_half, int rho_flag,
                                 MultiFab *delta_rhs,
                                 const MultiFab *const *betan,
                                 const MultiFab *const betanCC,
                                 const MultiFab *const *betanp1,
                                 const MultiFab *const betanp1CC) {
  diffuse_velocity(dt, be_cn_theta, rho_half, rho_flag, delta_rhs, 0, betan,
                   betanCC, betanp1, betanp1CC, 0);
}

void Diffusion::diffuse_velocity(
    Real dt, Real be_cn_theta, const MultiFab &rho_half, int rho_flag,
    MultiFab *delta_rhs, int rhsComp, const MultiFab *const *betan,
    const MultiFab *const betanCC, const MultiFab *const *betanp1,
    const MultiFab *const betanp1CC, int betaComp) {
  if (verbose)
  ...
  diffuse_tensor_velocity(dt, be_cn_theta, rho_half, rho_flag, delta_rhs,
                          rhsComp, betan, betanCC, betanp1, betanp1CC,
                          betaComp);
...
void Diffusion::diffuse_tensor_velocity(
    Real dt, Real be_cn_theta, const MultiFab &rho_half, int rho_flag,
    MultiFab *delta_rhs, int rhsComp, const MultiFab *const *betan,
    const MultiFab *const betanCC, const MultiFab *const *betanp1,
    const MultiFab *const betanp1CC, int betaComp) {
  ...
  if (allnull && be_cn_theta != 1)
    amrex::Abort("Diffusion::diffuse_tensor_velocity: Constant viscosity case "
                 "no longer supported separately. Must set non-zero beta.");
  ...
  const int soln_ng = 1;
  int flux_ng = 0;
  MultiFab Rhs(grids, dmap, AMREX_SPACEDIM, 0, MFInfo(),
               navier_stokes->Factory());
  MultiFab Soln(grids, dmap, AMREX_SPACEDIM, soln_ng, MFInfo(),
                navier_stokes->Factory());
  ...
  if (be_cn_theta != 1) {
...

    {
...
#ifdef AMREX_USE_EB
      auto const *ebf =
          &(dynamic_cast<EBFArrayBoxFactory const &>(navier_stokes->Factory()));
      MLEBTensorOp tensorop({navier_stokes->Geom()}, {grids}, {dmap}, info,
                            {ebf});
#else
      ...
      // pass to op
      tensorop.setDomainBC(
          {AMREX_D_DECL(mlmg_lobc[0], mlmg_lobc[1], mlmg_lobc[2])},
      ...
      // set coarse-fine BCs
      {
        ...
        if (level > 0) {
          ...
          AmrLevel::FillPatch(crse_ns, crsedata, ng, prev_time, State_Type,
                              Xvel, AMREX_SPACEDIM);
          tensorop.setCoarseFineBC(&crsedata, crse_ratio[0]);
        }
        AmrLevel::FillPatch(*navier_stokes, Soln, soln_ng, prev_time,
                            State_Type, Xvel, AMREX_SPACEDIM);
      ...
      int nghost(2);
      MultiFab Rhs_tmp(grids, dmap, AMREX_SPACEDIM, nghost, MFInfo(),
                       navier_stokes->Factory());
      Rhs_tmp.setVal(0.);
      ...
      if (NavierStokesBase::getRedistType() != "NoRedist") {
        ...
        single_level_weighted_redistribute(
            Rhs_tmp, Rhs, weights, 0, AMREX_SPACEDIM, navier_stokes->Geom(),
            use_wts_in_divnc);
      } else
#endif
      {
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
  ...
  for (MFIter mfi(Rhs, mfi_info); mfi.isValid(); ++mfi) {
    ...
    auto const &rho =
        (rho_flag == 1)
            ? rho_half.array(mfi)
            : navier_stokes->get_new_data(State_Type).array(mfi, Density);
    auto const &deltarhs =
        (has_delta_rhs) ? delta_rhs->array(mfi, rhsComp) : U_new.array(mfi);
    amrex::ParallelFor(bx, [rhs, unew, rho, deltarhs, has_delta_rhs,
                            dt] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
  ...
  // MLMG solution
  {
    ...
    // set up level BCs
    {
      ...
      AmrLevel::FillPatch(*navier_stokes, Soln, soln_ng, cur_time, State_Type,
                          Xvel, AMREX_SPACEDIM);
      tensorop.setLevelBC(0, &Soln);
    }

    {
      ...
      Real rhsscale = 1.0;
      const MultiFab &rho =
          (rho_flag == 1) ? rho_half : navier_stokes->get_new_data(State_Type);
      const int rho_comp = (rho_flag == 1) ? 0 : Density;
      computeAlpha(acoef, scalars, a, b, &rhsscale, nullptr, 0, rho_flag, &rho,
                   rho_comp);
    ...
    if (do_reflux && (level < finest_level || level > 0)) {
      ...
      if (level > 0) {
        for (int k = 0; k < AMREX_SPACEDIM; k++)
          viscflux_reg->FineAdd(*(tensorflux[k]), k, Xvel, Xvel, AMREX_SPACEDIM,
                                dt);
      }
      if (level < finest_level) {
        for (int d = 0; d < AMREX_SPACEDIM; d++)
          finer->viscflux_reg->CrseInit(*tensorflux[d], d, 0, Xvel,
                                        AMREX_SPACEDIM, -dt);
      }
    }
  }
...
void Diffusion::diffuse_Vsync(MultiFab &Vsync, Real dt, Real be_cn_theta,
                              const MultiFab &rho_half, int rho_flag,
                              const MultiFab *const *beta, int betaComp,
                              bool update_fluxreg) {
  AMREX_ASSERT(rho_flag == 1 || rho_flag == 3);
...
void Diffusion::diffuse_tensor_Vsync(MultiFab &Vsync, Real dt, Real be_cn_theta,
                                     const MultiFab &rho_half, int rho_flag,
                                     const MultiFab *const * /*beta*/,
                                     int /*betaComp*/, bool update_fluxreg) {
  AMREX_ASSERT(rho_flag == 1 || rho_flag == 3);

  if (verbose)
    amrex::Print() << "Diffusion::diffuse_tensor_Vsync ...\n";
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
  ...
  for (MFIter mfi(Rhs, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
    ...
    auto const &rho =
        (rho_flag == 1)
            ? rho_half.array(mfi)
            : navier_stokes->get_old_data(State_Type).array(mfi, Density);

    amrex::ParallelFor(
        bx, [rhs, rho] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
          for (int n = 0; n < AMREX_SPACEDIM; n++) {
...
void Diffusion::diffuse_Ssync(MultiFab &Ssync, int sigma, int nComp, Real dt,
                              Real be_cn_theta, const MultiFab &rho_half,
                              int rho_flag, MultiFab *const *flux, int fluxComp,
                              const MultiFab *const *beta, int betaComp,
                              const MultiFab *alpha, int alphaComp) {
  const int state_ind = sigma + AMREX_SPACEDIM;
  if (verbose) {
    for (int comp = 0; comp < nComp; ++comp) {
      amrex::Print() << "Diffusion::diffuse_Ssync lev: " << level << " "
                     << NavierStokesBase::get_desc_lst()[State_Type].name(
                            state_ind + comp)
                     << '\n';
    }
  }
  ...
  if (verbose > 1) {
    ...
    for (int comp = 0; comp < nComp; ++comp) {
      ...
      amrex::Print() << "Original max of Ssync of comp " << comp << ": "
                     << r_norm << '\n';
    }
  }
  //
...

  {
    ...
    computeAlpha(acoef, scalars, a, b, &rhsscale, alpha, alphaComp, rho_flag,
                 &rho, Rho_comp);
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(S_new, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
      ...
      amrex::ParallelFor(bx, nComp,
                         [ssync_arr, rho_arr] AMREX_GPU_DEVICE(
                             int i, int j, int k, int n) noexcept {
...
void Diffusion::computeAlpha(MultiFab &alpha, std::pair<Real, Real> &scalars,
                             Real a, Real b, Real *rhsscale,
                             const MultiFab *alpha_in, int alpha_in_comp,
                             int rho_flag, const MultiFab *rho, int rho_comp) {
...
#ifdef AMREX_USE_EB
void Diffusion::setBeta(MLEBABecLap &op, const MultiFab *const *beta,
                        int betaComp, int nComp) {
  ...
  op.setBCoeffs(0, amrex::GetArrOfConstPtrs(bcoeffs),
                MLLinOp::Location::FaceCentroid);
}

void Diffusion::setViscosity(MLEBTensorOp &tensorop,
                             const MultiFab *const *beta, int betaComp,
...
void Diffusion::setBeta(MLABecLaplacian &op, const MultiFab *const *beta,
                        int betaComp, int nComp) {
  Array<MultiFab, AMREX_SPACEDIM> bcoeffs{
      AMREX_D_DECL(MultiFab(*beta[0], amrex::make_alias, betaComp, nComp),
                   MultiFab(*beta[1], amrex::make_alias, betaComp, nComp),
...
void Diffusion::setViscosity(MLTensorOp &tensorop, const MultiFab *const *beta,
                             int betaComp) {
  Array<MultiFab, AMREX_SPACEDIM> face_bcoef{
      AMREX_D_DECL(MultiFab(*beta[0], amrex::make_alias, betaComp, 1),
                   MultiFab(*beta[1], amrex::make_alias, betaComp, 1),
...
//
void Diffusion::computeExtensiveFluxes(MLMG &a_mg, MultiFab &Soln,
                                       MultiFab *const *flux,
                                       const int fluxComp, const int ncomp,
                                       MultiFab *area, const Real fac) {
  ...
  auto const &ebfactory =
      dynamic_cast<EBFArrayBoxFactory const &>(Soln.Factory());
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
  ...
  for (MFIter mfi(Soln, mfi_info); mfi.isValid(); ++mfi) {
    ...
    if (flags.getType(amrex::grow(bx, 0)) == FabType::covered) {
      ...
      //
      AMREX_D_TERM(AMREX_PARALLEL_FOR_4D(ubx, ncomp, i, j, k, n,
                                         { fx(i, j, k, n) = COVERED_VAL; });
                   , AMREX_PARALLEL_FOR_4D(vbx, ncomp, i, j, k, n,
                                           { fy(i, j, k, n) = COVERED_VAL; });
                   , AMREX_PARALLEL_FOR_4D(wbx, ncomp, i, j, k, n,
      ...
      AMREX_D_TERM(
          AMREX_PARALLEL_FOR_4D(
              ubx, ncomp, i, j, k, n,
              { fx(i, j, k, n) *= fac * areax(i, j, k) * afrac_x(i, j, k); });
          , AMREX_PARALLEL_FOR_4D(
                vbx, ncomp, i, j, k, n,
                { fy(i, j, k, n) *= fac * areay(i, j, k) * afrac_y(i, j, k); });
          , AMREX_PARALLEL_FOR_4D(wbx, ncomp, i, j, k, n, {
            fz(i, j, k, n) *= fac * areaz(i, j, k) * afrac_z(i, j, k);
          }););
    } else
#endif
    {
      AMREX_D_TERM(
          AMREX_PARALLEL_FOR_4D(ubx, ncomp, i, j, k, n,
                                { fx(i, j, k, n) *= fac * areax(i, j, k); });
          , AMREX_PARALLEL_FOR_4D(vbx, ncomp, i, j, k, n,
                                  { fy(i, j, k, n) *= fac * areay(i, j, k); });
          , AMREX_PARALLEL_FOR_4D(wbx, ncomp, i, j, k, n, {
            fz(i, j, k, n) *= fac * areaz(i, j, k);
          }););
    }
  }
}
...
void Diffusion::getViscTerms(MultiFab &visc_terms, int src_comp, int comp,
  ...
  if (is_diffusive[comp]) {
    ...
    MultiFab visc_tmp(grids, dmap, 1, ng_visc, MFInfo(),
                      navier_stokes->Factory()),
    ...
    if (NavierStokesBase::getRedistType() != "NoRedist") {
      ...
      bool use_wts_in_divnc = true;
      single_level_weighted_redistribute(
          visc_tmp, visc_terms, weights, comp - src_comp, 1,
...
void Diffusion::getTensorViscTerms(MultiFab &visc_terms, Real time,
                                   const MultiFab *const *beta,
                                   const MultiFab *const betaCC, int betaComp) {
  int allthere;
  checkBeta(beta, allthere);
  ...
  if (ncomp < AMREX_SPACEDIM)
    amrex::Abort("Diffusion::getTensorViscTerms(): visc_terms needs at least "
                 "AMREX_SPACEDIM components");
  //
  // Before computing the godunov predictors we may have to
  // precompute the viscous source terms.  To do this we must
  // construct a Laplacian operator, set the coefficients and apply
  ...
  if (is_diffusive[src_comp]) {
    int ng = 1;
    MultiFab visc_tmp(grids, dmap, AMREX_SPACEDIM, 2, MFInfo(),
                      navier_stokes->Factory()),
        s_tmp(grids, dmap, AMREX_SPACEDIM, ng, MFInfo(),
              navier_stokes->Factory());
    visc_tmp.setVal(0.);
    ...
    // MLMG tensor solver
    {
      ...
      // set coarse-fine BCs
      {
        ...
        AmrLevel::FillPatch(*navier_stokes, s_tmp, ng, time, State_Type, Xvel,
                            AMREX_SPACEDIM);
    ...
    if (NavierStokesBase::getRedistType() != "NoRedist") {
      auto const &weights = ebf->getVolFrac();
      bool use_wts_in_divnc = true;
      single_level_weighted_redistribute(visc_tmp, visc_terms, weights, 0,
                                         AMREX_SPACEDIM, navier_stokes->Geom(),
                                         use_wts_in_divnc);
    } else
#endif
    {
...
void Diffusion::FillBoundary(BndryRegister &bdry, int state_ind, int dest_comp,
  ...
  MultiFab S(navier_stokes->boxArray(), navier_stokes->DistributionMap(),
             num_comp, nGrow, MFInfo(), navier_stokes->Factory());

  AmrLevel::FillPatch(*navier_stokes, S, nGrow, time, State_Type, state_ind,
                      num_comp);
...
void Diffusion::checkBeta(const MultiFab *const *beta, int &allthere,
                          int &allnull) {
  allnull = 1;
...
//      and diffusionType.
//
int Diffusion::set_rho_flag(const DiffusionForm compDiffusionType) {
  int rho_flag = 0;
  ...
  default:
    ...
    amrex::Abort(
        "An unknown NavierStokesBase::DiffusionForm was used in set_rho_flag");
  }

  return rho_flag;
}

void Diffusion::setDomainBC(std::array<LinOpBCType, AMREX_SPACEDIM> &mlmg_lobc,
                            std::array<LinOpBCType, AMREX_SPACEDIM> &mlmg_hibc,
                            int src_comp) {
  const BCRec &bc =
      NavierStokesBase::get_desc_lst()[State_Type].getBC(src_comp);
  for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
    if (parent->Geom(0).isPeriodic(idim)) {
      ...
      } else if (pbc == BCType::foextrap || pbc == BCType::hoextrap ||
