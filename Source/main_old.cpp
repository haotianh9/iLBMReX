...
void taylor_green(amrex::Real t, amrex::MultiFab &rho, amrex::MultiFab &ux,
                  ...
                  int Nx, int Ny, amrex::GpuArray<amrex::Real, 2> dx,
                  int Nghost) {
  ...
  //                            double X = i + 0.5;
  //                            double Y = j + 0.5;
  //                            ux_local(i, j, 0) = -u_max * sqrt(ky / kx) *
  //                            cos(kx * X) * sin(ky * Y) * exp(-1.0 * t / td);
  //                            uy_local(i, j, 0) = u_max * sqrt(kx / ky) *
  //                            sin(kx * X) * cos(ky * Y) * exp(-1.0 * t / td);
  //                            double P = -0.25 * rho0 * u_max * u_max * ((ky /
  //                            kx) * cos(2.0 * kx * X) + (kx / ky) * cos(2.0 *
  //                            ky * Y)) * exp(-2.0 * t / td); rho_local(i, j,
  //                            0) = rho0 + 3.0 * P;
  //                         //    amrex::Print() << "i,j: " << i << " , " << j
  //                         << "  rho: " << rho_local(i, j, 0) << "\n";
...
void init_equilibrium(amrex::MultiFab &f1, amrex::MultiFab &rho,
                      amrex::MultiFab &ux, amrex::MultiFab &uy, int Nghost) {
  ...
  amrex::ParallelFor(
      f1, [=] AMREX_GPU_DEVICE(int nbx, int i, int j, int k) noexcept {
        for (unsigned int i_dir = 0; i_dir < ndir; ++i_dir) {
          ...
          f1_arrs[nbx](i, j, k, i_dir) =
              wi[i_dir] * rho_arrs[nbx](i, j, k) *
...
void compute_rho_u(amrex::MultiFab &f1, amrex::MultiFab &rho,
                   amrex::MultiFab &ux, amrex::MultiFab &uy, int Nghost) {
  ...
  amrex::ParallelFor(
      f_old, [=] AMREX_GPU_DEVICE(int nbx, int i, int j, int k) noexcept {
        ...
        for (unsigned int i_dir = 0; i_dir < ndir; ++i_dir) {
          ...
          double feq = wi[i_dir] * rho_arrs[nbx](i, j, k) *
                       (1.0 + 3.0 * cidotu + 4.5 * cidotu * cidotu -
                        1.5 * (ux_arrs[nbx](i, j, k) * ux_arrs[nbx](i, j, k) +
                               uy_arrs[nbx](i, j, k) * uy_arrs[nbx](i, j, k)));
...
int main(int argc, char *argv[]) {
  amrex::Initialize(argc, argv);
  {
    ...
    // inputs parameters
    {
      ...
      // pp.query means we optionally need the inputs file to have it - but we
      // must supply a default here
      amrex::ParmParse pp;
      ...
      // Default nsteps to 10, allow us to set it to something else in the
      // inputs file
      NSTEP = 10;
      pp.query("NSTEP", NSTEP);
      NSTEP = NSTEP * scale * scale;
      amrex::Print() << "NSTEP: " << NSTEP << "\n";
      // Default plot_int to -1, allow us to set it to something else in the
      // inputs file
    ...
    amrex::RealBox real_box({0., 0.}, {Nx, Ny});
    ...
    // we allocate two f multifabs; one will store the old state, the other the
    // new.
    ...
    if (plot_int > 0) {
      ...
      WriteSingleLevelPlotfile(pltfile, plotfile_mf,
                               {"rho", "ux", "uy", "f1", "f2", "f3", "f4", "f5",
                                "f6", "f7", "f8", "f9"},
                               geom, time, 0);
    }
    ...
    for (int step = 1; step <= NSTEP; ++step) {
      ...
      if (plot_int > 0) {
        if (step % plot_int == 0) {
          ...
          const std::string &pltfile = amrex::Concatenate("plt", step, 5);
          amrex::MultiFab plotfile_mf(ba, dm, ndir + 3, Nghost,
                                      amrex::MFInfo());
